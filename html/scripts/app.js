(function(){
	...
	let renderContacts = function(){
		...
		if(contacts.length > 0) {
			...
			contactHTML = '<div class="contact no-item online"><p class="no-item">没有联系人</p></div>';
		}
	...
	let addMessage = function(phoneNumber, pmessage, pposition, panonyme, pjob){
		...
		if(messages.length >  0) {
			...
			messageHTML = '<div class="message no-item"><p class="no-item">没有短信息</p></div>';
		}
	...
	let renderSpecialContacts = function(){
		...
		$('#phone .menu .home').html(
			'<li class="phone-icon" id="phone-icon-rep">联系人</li>' +
			'<li class="phone-icon" id="phone-icon-msg">短信</li>'
		);
